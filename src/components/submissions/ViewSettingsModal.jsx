...
const ALL_STATUSES = ["submitted","under_review","interviewing","rejected","hired","withdrawn"];

export default function ViewSettingsModal({
  open,
  onClose,
  initial = { name: "", view_type: "list", columns: ALL_STATUSES, filters: { status: [] }, visibility: "private" },
  onSave
}) {
  ...
  const toggleStatus = (st) => {
    if (data.columns.includes(st)) {
      setData({ ...data, columns: data.columns.filter(s => s !== st) });
    } else {
      setData({ ...data, columns: [...data.columns, st] });
    }
  };
        ...
        <CardContent className="space-y-6">
          ...
          {data.view_type === "kanban" && (
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Tip: Checked statuses appear as columns (left-to-right in the order above).</p>
            </div>
          )}

          <div>
            <Label className="mb-2 block">Filters (applies to both list and kanban)</Label>
            <div className="grid grid-cols-2 gap-2">
              {ALL_STATUSES.map(st => (
                <label key={st} className="flex items-center gap-2 text-sm">
                  <Checkbox
                    checked={(data.filters?.status || []).includes(st)}
                    onCheckedChange={() => toggleFilterStatus(st)}
                  />
                  <span className="capitalize">{st.replace("_"," ")}</span>
                </label>
              ))}
